import { Category, Product } from "@/assets/products";
...
export default function AllItems() {
  ...
  const minPrice = useMemo(() => {
    ...
    return Math.floor(Math.min(...baseProducts.map((item) => item.price)));
  }, [baseProducts]);

  const maxPrice = useMemo(() => {
    if (!baseProducts.length) return 0;
    return Math.ceil(Math.max(...baseProducts.map((item) => item.price)));
  }, [baseProducts]);
  ...
  const productsToShow = useMemo(() => {
    ...
    if (filterItem === "Best Deals") {
      ...
      data = data
        .filter((item) => item.isAvailable && item.isPopular)
        .sort((a, b) => b.rating - a.rating);
    } else if (filterItem !== "All" && filterItem !== "All Items") {
      data = data.filter(
        (item) => item.isAvailable && item.category === filterItem,
      );
    }
    ...
    if (selectedSizes.length) {
      data = data.filter((product) =>
        product.sizes.some((size) => selectedSizes.includes(size.value)),
      );
    }
    ...
    if (selectedAvailability === "instock") {
      data = data.filter((product) =>
        product.branches.some((branch) => branch.available && branch.stock > 0),
      );
    }

    if (selectedAvailability === "unavailable") {
      data = data.filter((product) =>
        product.branches.every((branch) => !branch.available),
      );
    }
  ...
  const renderItem: SectionListRenderItem<Product[], Section> = useCallback(
    ({ item, section, index }) => {
      ...
      return (
        ...
        >
          <FoodCard item={item[0]} index={index} isVertical />

          {item[1] ? (
            <FoodCard item={item[1]} index={index} isVertical />
          ) : (
            <View style={{ width: "48%" }} />
          )}
        </View>
